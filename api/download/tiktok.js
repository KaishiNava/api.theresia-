export default async function handler(req, res) {
  ...
  try {
    const tikwmApiUrl = `https://www.tikwm.com/api/?url=${encodeURIComponent(targetUrl.toString())}`;
    ...
    if (!response.ok) {
      return res.status(502).json({ status: false, message: "Gagal menghubungi TikWM" });
    }
    ...
    if (result.code !== 0 || !result.data) {
      return res.status(400).json({
        status: false,
        message: result.msg || "Gagal mengambil media TikTok."
      });
    }
    ...
    if (download) {
      if (!videoNowm) {
        return res.status(404).json({ status: false, message: "URL video tidak tersedia" });
      }
      ...
      if (!videoResponse.ok) {
        return res.status(502).json({ status: false, message: "Gagal mengambil video dari provider" });
      }
    ...
    return res.status(200).json({
      ...
      data: {
        title: videoData.title || "TikTok Media",
        ...
        video_wm: videoWm,
        audio: audio,
        download_url: `${baseUrl}/api/download/tiktok?url=${encodeURIComponent(targetUrl.toString())}&download=1`
      }
    });
